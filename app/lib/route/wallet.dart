...
import '../../bindings_generated_api.dart' show AppHandle, FiatRate, NodeInfo;
...
class WalletPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final fiatRates = BehaviorSubject<FiatRate?>.seeded(null);
    final nodeInfos = BehaviorSubject<NodeInfo?>.seeded(null);

    // A stream of `BalanceState`s that gets updated when `nodeInfos` or
    // `fiatRates` are updated. Since it's wrapped in a `StateStream`, it also
    // avoids widget rebuilds if new state == old state.
    final balanceStates = Rx.combineLatest2(
      nodeInfos.map((nodeInfo) => nodeInfo?.localBalanceMsat),
      fiatRates.map((fiatRates) => fiatRates?.rate),
    ...
    refreshRx.asyncMap((_) => this.app.nodeInfo()).listen(
          nodeInfos.add,
          onError: (err) => debugPrint("nodeInfos: error: $err"),
        );

    // on refresh, update fiat rate
    refreshRx.asyncMap((_) => this.app.fiatRate(fiat: fiatName)).listen(
          fiatRates.add,
