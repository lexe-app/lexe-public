...
void main() {
  ...
  test("date_format.formatDate", () async {
    ...
    expect(date_format.formatDate(now: now, then: days2, locale: "en_US"),
        "2 days ago");
    expect(date_format.formatDate(now: now, then: days2, locale: "nb"),
        "2 dager ago");
    expect(date_format.formatDate(now: now, then: days2, locale: "fr"),
        "2 jours ago");

    // -15h 5m 3s = "15 hours ago"
    final hours15 = dateTimeFromUnix(1687330792000);
    expect(date_format.formatDate(now: now, then: hours15, locale: "en_US"),
        "15 hours ago");
    expect(date_format.formatDate(now: now, then: hours15, locale: "nb"),
        "15 timer ago");
    expect(date_format.formatDate(now: now, then: hours15, locale: "fr"),
        "15 heures ago");

    // -5m 3s = "5 minutes ago"
    final min5 = dateTimeFromUnix(1687384792000);
    expect(date_format.formatDate(now: now, then: min5, locale: "en_US"),
        "5 minutes ago");
    expect(date_format.formatDate(now: now, then: min5, locale: "nb"),
        "5 minutter ago");
    expect(date_format.formatDate(now: now, then: min5, locale: "fr"),
        "5 minutes ago");

    // -15 secs = "just now"
    final secs15 = dateTimeFromUnix(1687385080000);
    expect(date_format.formatDate(now: now, then: secs15, locale: "en_US"),
        "just now");
    ...
    expect(
        date_format.formatDate(now: now, then: apr7, locale: "en_US"), "Apr 7");
    expect(
        date_format.formatDate(now: now, then: apr7, locale: "nb"), "7. apr.");
    expect(
        date_format.formatDate(now: now, then: apr7, locale: "fr"), "7 avr.");

    // -180d = December 23, 2022 = "Dec 23"
    final dec23 = dateTimeFromUnix(1671818392000);
    expect(date_format.formatDate(now: now, then: dec23, locale: "en_US"),
        "Dec 23");
    expect(date_format.formatDate(now: now, then: dec23, locale: "nb"),
        "23. des.");
    expect(
        date_format.formatDate(now: now, then: dec23, locale: "fr"), "23 déc.");

    // -200d = December 3 2022 = "12/03/22"
    final dec3_22 = dateTimeFromUnix(1670090392000);
    expect(date_format.formatDate(now: now, then: dec3_22, locale: "en_US"),
        "12/3/2022");
    expect(date_format.formatDate(now: now, then: dec3_22, locale: "nb"),
        "3.12.2022");
    expect(date_format.formatDate(now: now, then: dec3_22, locale: "fr"),
        "03/12/2022");

    // -654d = September 5, 2021 = "09/05/21"
    final sep5_22 = dateTimeFromUnix(1630864792000);
    expect(date_format.formatDate(now: now, then: sep5_22, locale: "en_US"),
        "9/5/2021");
    expect(date_format.formatDate(now: now, then: sep5_22, locale: "nb"),
        "5.9.2021");
    expect(date_format.formatDate(now: now, then: sep5_22, locale: "fr"),
        "05/09/2021");
  });
