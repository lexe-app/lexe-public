...
impl LexeKeysManager {
    ...
    ///
    /// Assumes `channel_sweep_address` was generated by our BDK wallet.
    pub fn unchecked_init<R: Crng>(
        rng: &mut R,
        root_seed: &RootSeed,
        channel_sweep_address: bitcoin::Address,
    ) -> anyhow::Result<Self> {
        ...
        let (destination_script, shutdown_scriptpubkey) =
            Self::shutdown_scripts_from_sweep_address(channel_sweep_address)
    ...
    fn shutdown_scripts_from_sweep_address(
        channel_sweep_address: bitcoin::Address,
    ) -> anyhow::Result<(bitcoin::ScriptBuf, ShutdownScript)> {
        let destination_script = channel_sweep_address.script_pubkey();
        let shutdown_scriptpubkey = match channel_sweep_address.payload {
...
impl SignerProvider for LexeKeysManager {
    ...
    fn get_destination_script(
        ...
        // TODO(max): LDK notes we should return a different value each time
        // this method is called, for privacy. We can use `channel_keys_id` to
        // derive a unique value for each channel.
...
mod test {
    ...
    fn test_rootseed_keysmanager_derivation_equivalence() {
        ...
        let mk_sweep_address = || {
            // This payload was generated by putting this in node init:
            // println!("{:?}", wallet.get_address().await.unwrap().payload);
            let version = address::WitnessVersion::V0;
            let program = vec![
                71u8, 167, 97, 203, 58, 128, 110, 64, 72, 83, 112, 4, 245, 234,
                35, 251, 92, 105, 219, 42,
            ];
            let witness_program =
                address::WitnessProgram::new(version, program).unwrap();
            let payload = address::Payload::WitnessProgram(witness_program);
        ...
        proptest!(|(root_seed in any_root_seed, mut rng in any_rng)| {
            ...
            let channel_sweep_address = mk_sweep_address();
            let keys_manager = LexeKeysManager::unchecked_init(
                &mut rng, &root_seed, channel_sweep_address
            )
            .unwrap();
