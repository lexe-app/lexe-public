...
///
/// We're aiming to exactly match the fee that LDK actually negotiates during
/// coop close. The negotiation flow is:
///
...
fn our_close_tx_fees_sats(
    ...
    //
    // Our LN balance (`amount_satoshis + transaction_fee_satoshis`) includes
    // this rounding sat (via `transaction_fee_satoshis = commit_fee +
    // msat_round`), but the closing tx output to us is computed from
    // `floor(value_to_self_msat / 1000)` which doesn't include it.
    //
