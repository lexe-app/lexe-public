...
use crate::{constants, ed25519, rng::Crng, root_seed::RootSeed};
...
pub fn node_run_tls_config<R: Crng>(
    ...
    let ca_cert = certs::CaCert::from_key_pair(ca_cert_key_pair)
    ...
    let node_cert = certs::NodeCert::from_key_pair(node_key_pair, dns_names)
...
pub fn client_run_tls_config<R: Crng>(
    rng: &mut R,
    ...
    let server_cert_verifier = RunCertVerifier {
        lexe_verifier: super::lexe_verifier(lexe_ca_cert)?,
        node_verifier,
    };

    let client_cert_resolver = RunCertResolver::new(rng, &ca_cert)
...
/// certs of a running node.
struct RunCertVerifier {
...
impl rustls::client::ServerCertVerifier for RunCertVerifier {
...
impl RunCertResolver {
    ...
    fn new<R: Crng>(
        ...
        let client_cert =
            certs::ClientCert::from_key_pair(client_cert_key_pair)
...
impl rustls::client::ResolvesClientCert for RunCertResolver {
    fn resolve(
        &self,
        ...
        let remote_wants_client_cert = acceptable_issuers
            .iter()
            .any(|der| certs::CaCert::is_matching_issuer_der(der));
...
mod test {
    ...
    async fn test_tls_handshake_succeeds() {
        ...
        let client = async move {
            ...
            let lexe_root = certs::CaCert::from_key_pair(lexe_root_key_pair)
                .unwrap()
                .serialize_der_signed()
                .unwrap();
