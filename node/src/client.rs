...
use lexe_api::{
    ...
    models::{
        command::{
            ClaimPaymentAddress, GetNewPayments, GetUpdatedPaymentMetadata,
            GetUpdatedPayments, LxPaymentIdStruct, PaymentAddress,
...
impl NodeBackendApi for NodeBackendClient {
    ...
    async fn upsert_payment_metadata(
        ...
        let req = self
            .rest
            .put(format!("{backend}/node/v1/payments/metadata"), &metadata)
            .bearer_auth(&auth);
        self.rest.send(req).await
    }
