...
///
/// DNS-over-HTTPS is robust on VPNs, unlike direct DNS which is often blocked
/// and leaks queries to your ISP.
...
pub(super) async fn resolve_bip353_fqdn(
    ...
    let dnssec_proof = get_dnssec_proof_dns_over_https(&dns_name, doh_endpoint)
        .await
        .context("Failed to get DNS proof over DNS-over-HTTPS")?;

    // NOTE: This BIP353 DNSSEC proof can be stored and later used to prove the
    // link between a BIP 353 human readable name and the resolved payment URI
    // as part of a proof of payment. We're going to punt on this until and
    // unless a user asks for it.
    let _ = dnssec_proof;

    let bip321_uri = dnssec_proof_to_uri(&dns_name, &dnssec_proof)
...
async fn get_dnssec_proof_dns_over_https(
    ...
    // If using the Google DOH endpoint, we can trust only Google's root CAs.
    ...
    while let Some(query) = pending_queries.pop() {
        let body = send_doh_query(&client, doh_endpoint, &query).await?;
        ...
        match proof_builder.process_response(&answer) {
            ...
            Err(e) => bail!("Error processing DNS-over-HTTPS response: {e:#}"),
        }
    }

    let (dnssec_proof, _ttl) = proof_builder
        .finish_proof()
        .map_err(|()| anyhow!("Failed to build DNSSEC proof from responses"))?;
...
fn dnssec_proof_to_uri(
    ...
    // Parse
    let rrs =
        dnssec_prover::ser::parse_rr_stream(dnssec_proof).map_err(|()| {
    ...
    // Find the BIP353 TXT record. Per BIP353, we must ignore any TXT records
    // that don't start with "bitcoin:" (case-insensitive), and there must be
    // exactly one TXT record that starts with "bitcoin:".
    ...
    let mut bitcoin_txt_records = resolved_rrs
        ...
        // Only consider TXT records starting with "bitcoin:" (case-insensitive)
        .filter_map(|txt_str| {
    ...
    Bip321Uri::parse(&bip321_uri_str)
        ...
        .context("Could not parse 'bitcoin:' payment uri from DNS TXT record")
}
...
mod test {
    ...
    async fn test_bip353_bluematt_dns_over_https() {
        ...
        let payment_methods = tokio::time::timeout(
            ...
            resolve_bip353_fqdn(bip353_fqdn, GOOGLE_DOH_ENDPOINT),
        )
        .await
        .expect("Timed out")
        .unwrap();
    ...
    async fn test_bip353_bluematt_direct_dns() {
        ...
        let bip321_uri = dnssec_proof_to_uri(&dns_name, &dnssec_proof).unwrap();
        debug!("Resolved BIP353 address: {bip321_uri}");
